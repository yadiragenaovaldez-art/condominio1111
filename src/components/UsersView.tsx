...
export default function UsersView({
  ...
  const isMainAdmin = currentUser.username.toLowerCase() === "admin" || currentUser.id === "user-admin";
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-12 select-none animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        {/* Left Side: Create / Edit Form or Info details if not Admin */}
          ...
          {!isMainAdmin ? (
            <div className="space-y-5 text-left py-2">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[9px] text-amber-600 font-extrabold uppercase tracking-widest leading-none">
                  Privilegio de Creador Restringido
                </p>
              </div>
              <p className="text-xs text-slate-500 leading-relaxed font-semibold">
                Solo el usuario <strong className="text-slate-800 font-bold">Administrador Principal (Admin)</strong> de condominio tiene autorización para agregar operadores locales de cobro, redefinir contraseñas o modificar módulos de acceso.
              </p>
              <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl space-y-3">
                <div className="flex items-center gap-2">
                  <UserCheck className="w-4 h-4 text-emerald-500" />
                  <span className="text-[10px] text-slate-600 font-bold uppercase tracking-wider">Tu Sesión Actual</span>
                </div>
                <div className="text-xs pl-6 leading-tight space-y-1">
                  <span className="text-slate-700 font-black block">{currentUser.name}</span>
                  <span className="text-slate-400 font-mono text-[9px] block">@{currentUser.username} | {currentUser.cargo}</span>
                </div>
              </div>
              <p className="text-[9.5px] text-slate-400 font-medium italic mt-2">
                * Las cuentas de operadores creadas por el Administrador pueden ingresar inmediatamente al sistema de forma local sin requerir configuraciones de correos en Firebase.
              </p>
            </div>
          ) : (
            <>
